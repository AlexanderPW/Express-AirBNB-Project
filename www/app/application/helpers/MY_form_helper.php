...
function form_countries($name, $value, $include_blank = FALSE, $attrs='') {
    $country_list = array(
        'US'=>'United States',
        'AF'=>'Afghanistan',
        'AL'=>'Albania',
...
function form_city($name, $value = '', $attrs = '') {
    $CI =& get_instance();
    $CI->load->database();

    $options = array();
    $query = $CI->db->order_by('city', 'asc')->get('city');
    foreach ($query->result() as $row) {
        $options[$row->id] = $row->city.", ".$row->state;
    }

    $options[''] = 'Featured City';
