<form action="" method="post" id="listing-form" class="std-form">
    <h3><% if (uuid) { %>Edit Submarket<% } else { %>Add Submarket to <%= orig_city.city %><% } %></h3>
    <% state = orig_city.state %>
       <div class="row">
        <div class="col-lg-8">
            <div class="form-group">
                <label class="sr-only" for="city">City</label>
                <input type="text" class="form-control" id="city" placeholder="City"
                       name="city" value="<% if (city) { %><%=city%><% } else { %><%= submarket[0].city %><% } %>"
                       data-rule-required="true">
                        <input type="hidden" for="cityID" class="form-control" id="cityID" name="cityID" value="<%=newuuid%>"/>
            </div>
            <div class="form-group">
                <label class="sr-only" for="state">State</label>
                <?= form_states('state', '<%=orig_city.state%>', false, "class='form-control'"); ?>
            </div>
...
<% } %>
    ...
    <div class="row">
        <div class="col-lg-12 submit-container">
            <a href="#delete_modal1" data-toggle="modal" class="pull-left btn btn-sm"><i class="glyphicons bin"></i>
                Delete</a>
...
<div class="modal fade" id="delete_modal1">
