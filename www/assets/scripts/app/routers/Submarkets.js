...
var subListingRouter = subBaseRouter.extend({
    container: '#backbone-container2',
    ...
    routes: {
        ...
        'submarkets': 'listing_details',
        'submarket/*path': 'listing_details',

        'submarkets/*path': 'listings',
    ...
    _close_views : function() {
        /* Kill the old view */
        if (this.sublistingView) {
            this.sublistingView.close();
        }
        if (this.sublistingsView) {
            this.sublistingsView.close();
        }
    },
    ...
    _get_fragments : function() {
        var url = Backbone.history.fragment.replace('submarkets', '');
    ...
    listings: function () {
        ...
        if(this.sublistingsView) {
            ...
            this.listingsView = new SubListingsView();
...
app.sublistingRouter = new subListingRouter;
jQuery(document).ready(function () {
    $ = jQuery.noConflict();
    app.sublistingRouter.start();
});
