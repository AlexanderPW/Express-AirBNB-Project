ListingsSidebarView = BaseView.extend({
    id: 'listings-sidebar',
    className: 'listings-sidebar',
    template_name: 'listings-sidebar',
    ...
    render: function () {
        ...
        if(app.listingRouter.url_data && app.listingRouter.url_data.zipcode){
            this.$el.find('#cityzip').val(app.listingRouter.url_data.zipcode);
        }
        if(app.listingRouter.url_data && app.listingRouter.url_data.city){
            this.$el.find('#cityzip').val(decodeURIComponent(app.listingRouter.url_data.city));
        }
        if(app.listingRouter.url_data && app.listingRouter.url_data.range){
            this.$el.find('#range').val(app.listingRouter.url_data.range);
        }
        if(app.listingRouter.url_data && app.listingRouter.url_data.state){
            this.$el.find('#state').val(app.listingRouter.url_data.state);
        }
        if(app.listingRouter.url_data && app.listingRouter.url_data.mode) {
            this.$el.find('#mode').val(app.listingRouter.url_data.mode);
        }

        if(app.listingRouter.listings_type=='featured') {
        ...
        if(app.listingRouter.url_data['mode']===app.listingRouter.LISTING_MODE_MAP) {
    ...
    update_sidebar_from_results : function() {
        ...
        me.$el.find('#range').val(app.listingRouter.range);

        if(app.listingRouter.amenity_counts.length>0) {
            _.each(app.listingRouter.amenity_counts, function(amenity_count) {
    ...
    change_amenities : function(event) {
        app.listingRouter.chosen_amenities = [];
        this.$el.find('.amenities input[type="checkbox"]:checked').each(function(checkbox) {
            app.listingRouter.chosen_amenities.push($(this).val());
        });
    ...
    change_bedrooms : function(event) {
        app.listingRouter.chosen_bedrooms = this.$el.find('.bedrooms input[type="radio"]:checked').val();
    ...
    change_listings_type : function(event) {
        app.listingRouter.listings_type = this.$el.find('.listings-type input[type="radio"]:checked').val();
    ...
    map_view : function(event) {
        ...
        this.options.parent.switchView(app.listingRouter.LISTING_MODE_MAP);
        this.$el.find('#mode').val(app.listingRouter.listing_mode);
        return false;
    },
    ...
    list_view : function(event) {
        ...
        this.options.parent.switchView(app.listingRouter.LISTING_MODE_LIST);
        this.$el.find('#mode').val(app.listingRouter.listing_mode);
        return false;
    }
});